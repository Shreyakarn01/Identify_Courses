...
public class ExtentReportManager implements ITestListener{
	...
	public void onStart(ITestContext testContext){ //testContext is which "test cases"(in testCases package) got executed is captured
		...
		*/
		...
		sparkReporter.config().setDocumentTitle("Identify_Courses Automation Report"); // Title of report
		sparkReporter.config().setReportName("Identify_Courses Functional Testing"); // name of the report
		sparkReporter.config().setTheme(Theme.DARK);
		
		//populating common info in the report
		extent = new ExtentReports();
		extent.attachReporter(sparkReporter);
		extent.setSystemInfo("Application", "Identify_Courses");
